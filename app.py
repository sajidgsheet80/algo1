...
function startAuto() {
    if (!window.autoRefresh) { window.autoRefresh = true; autoRefreshLoop(); }
}
function stopAuto() { window.autoRefresh = false; }
...
function autoRefreshLoop() {
    ...
    fetch("/fetch").then(res => res.json()).then(data => {
        updateTable(data);
        setTimeout(autoRefreshLoop, 5000);
    });
}
</script>
</body>
...
def fetch_option_chain_data():
    ...
    try:
        ...
        pivots = []
        for field, tag in [("ltp", "LTP"), ("volume", "VOLUME"), ("open_interest", "OI"), ("chng_in_oi", "OI_CHANGE")]:
            if field in df.columns:
                piv = df.pivot_table(index="strike_price", columns="option_type", values=field, aggfunc="first").reset_index()
                if "CE" in piv.columns: piv = piv.rename(columns={"CE": f"{tag}_CE"})
                if "PE" in piv.columns: piv = piv.rename(columns={"PE": f"{tag}_PE"})
                pivots.append(piv)
        ...
        if not strike_prices: strike_prices.extend(result["strike_price"].tolist())

        rows = []
        for row in result.itertuples():
            now = time.time()
            history[row.strike_price]["CE_LTP"].append((now, getattr(row, "LTP_CE", 0)))
            history[row.strike_price]["PE_LTP"].append((now, getattr(row, "LTP_PE", 0)))
            history[row.strike_price]["CE_VOL"].append((now, getattr(row, "VOLUME_CE", 0)))
            history[row.strike_price]["PE_VOL"].append((now, getattr(row, "VOLUME_PE", 0)))
            history[row.strike_price]["CE_OI"].append((now, getattr(row, "OI_CHANGE_CE", 0)))
            history[row.strike_price]["PE_OI"].append((now, getattr(row, "OI_CHANGE_PE", 0)))

            def check_5min(hist):
                if len(hist) < 2: return "No"
                old_time, old_val = hist[0]
                new_time, new_val = hist[-1]
                return "Yes" if new_time - old_time >= 300 and new_val > old_val else "No"
...
def fetch():
    if fyers is None:
        return jsonify([["Please login first"] + [""] * 12])
...
def index():
    ...
    if fyers is None:
        rows = [["Please login first"] + [""] * (len(columns)-1)]
    else:
